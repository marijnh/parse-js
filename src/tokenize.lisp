                     ...
                     (let ((digs (digits 10)))
                       (if (string= digs "")
                           ...
                           (loop (handler-case
                                   ...
                                   (floating-point-underflow () (setf digs (subseq digs 0 (1- (length digs)))))))))
                     (when (equal body "") (ret nil)))
                 (when (and (find (peek) "eE") (next))
                   (let ((neg-expt (and (find (peek) "+-") (eql (next) #\-))))
                     (let ((digs (digits 10)))
                       (when (equal digs "") (ret nil))
                       (setf expt (if neg-expt (- (parse-integer digs)) (parse-integer digs))))))
                 (handler-case (ret (* base (expt 10d0 expt)))
