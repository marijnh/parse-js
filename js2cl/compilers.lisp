...
(compiler :binary (op left right)
  (let ((left (compile left))
        (right (compile right)))
    (ecase op
      (:|\|\|| `(if (or (%%bool ,left) (%%bool ,right)) :true :false))
      (:&& `(if (and (%%bool ,left) (%%bool ,right)) :true :false))
      (:|\|| `(logior ,left ,right))
      (:^ `(logxor ,left ,right))
      (:& `(logand ,left ,right))
      (:== `(== ,left ,right))
      (:=== `(=== ,left ,right))
      (:!== `(if (eq (== ,left ,right) :true) :false :true))
      (:!=== `(if (eq (=== ,left ,right) :true) :false :true))
      (:< `(%< ,left ,right))
      (:> `(%> ,left ,right))
      (:<= `(if (eq (%> ,left ,right) :true) :false :true))
      (:>= `(if (eq (%< ,left ,right) :true) :false :true))
      (:in `(if (nth-value 1 (gethash ,left ,right)) :true :false))
