...
(defun/defs parse-js (stream &optional (strict-semicolons t))
  ...
  (def expect-token (type val)
    (if (tokenp token type val)
        (next)
        (error* "Unexpected '~a' -- expected ~a." (token-id token) val)))
  (def expect (punc)
    ...
    (case (token-type token)
      ...
      (:keyword
       (case (prog1 (token-value token) (next))
         ...
         (:function (function*))
         (:if (if*))
         (:return (let ((val (expression))) (semicolon) (as :return val)))
  ...
  (def break/cont (type)
    (let ((name nil))
      (when (token-type-p token :name)
        ...
        (next))
      (unless (member name labels :test #'string=) (error* "~a without matching loop or switch statement." type))
      (semicolon)
      (as type name)))
  ...
  (def for* ()
    ...
    (if (and (token-type-p token :name) (tokenp (peek) :operator :in))
        (let ((name (token-value token)))
          ...
          (let ((obj (expression)))
            (expect #\))
            (as :for-in name obj (statement))))
        (with-defs
          (def init (cond ((tokenp token :punc #\;) nil)
                          ((tokenp token :keyword :var) (statement))
                          (t (expression))))
          (semicolon)
          ...
          (expect #\))
          (as :for init test step (statement)))))
  ...
  (def try* ()
    (let ((body (expression)) catch finally)
  ...
  (def expr-atom ()
    (cond ((and (token-type-p token :operator) (member (token-value token) *unary-prefix*))
           (next)
           (as :unary-prefix (token-value token) (expr-atom)))
    ...
    (let ((left (maybe-conditional)))
      (if (and (token-type-p token :operator) (member (token-value token) *assignment*))
          (if (is-assignable left)
              (as :assign (token-value token) left (progn (next) (maybe-conditional)))
...
(defun parse-js-string (string &optional (strict-semicolons t))
